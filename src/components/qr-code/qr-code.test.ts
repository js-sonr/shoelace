...
import type NuQrCode from './qr-code.js';

const getCanvas = (qrCode: NuQrCode): HTMLCanvasElement => {
...
const expectCanvasToHaveAriaLabel = (qrCode: NuQrCode, expectedLabel: string): void => {
...
const getQrCodeColors = (qrCode: NuQrCode): QrCodeColors => {
  const canvas = getCanvas(qrCode);
...
const expectQrCodeColorsToBe = (qrCode: NuQrCode, expectedColors: QrCodeColors): void => {
...
describe('<nu-qr-code>', () => {
  it('should render a component', async () => {
    const qrCode = await fixture<NuQrCode>(html` <nu-qr-code value="test data"></nu-qr-code>`);
  ...
  it('uses the label if given', async () => {
    const qrCode = await fixture<NuQrCode>(html` <nu-qr-code value="test data" label="test label"></nu-qr-code>`);

    expectCanvasToHaveAriaLabel(qrCode, 'test label');
  });

  it('sets the correct color for the qr code', async () => {
    const qrCode = await fixture<NuQrCode>(html` <nu-qr-code value="test data" fill="red"></nu-qr-code>`);

    expectQrCodeColorsToBe(qrCode, { foreground: red, background: white });
  });

  it('sets the correct background for the qr code', async () => {
    const qrCode = await fixture<NuQrCode>(
      html` <nu-qr-code value="test data" fill="red" background="blue"></nu-qr-code>`
    );
  ...
  it('has the expected size', async () => {
    const qrCode = await fixture<NuQrCode>(html` <nu-qr-code value="test data" size="100"></nu-qr-code>`);
