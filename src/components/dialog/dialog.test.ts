...
import type NuDialog from './dialog.js';

describe('<nu-dialog>', () => {
  it('should be visible with the open attribute', async () => {
    const el = await fixture<NuDialog>(html`
      <nu-dialog open>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</nu-dialog>
    `);
  ...
  it('should not be visible without the open attribute', async () => {
    const el = await fixture<NuDialog>(html`
      <nu-dialog>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</nu-dialog>
    `);
    const base = el.shadowRoot!.querySelector<HTMLElement>('[part~="base"]')!;
  ...
  it('should allow initial focus to be set', async () => {
    const el = await fixture<NuDialog>(html` <nu-dialog><input /></nu-dialog> `);
  ...
  it('should close when pressing Escape', async () => {
    const el = await fixture<NuDialog>(html` <nu-dialog open></nu-dialog> `);
    const hideHandler = sinon.spy();
  ...
  it('should properly cycle through tabbable elements when sl-dialog is used in a shadowRoot', async () => {
    class AContainer extends LitElement {
      get dialog() {
        return this.shadowRoot?.querySelector('nu-dialog');
      }
      ...
      render() {
        return html`
          ...
          <nu-button @click=${this.openDialog}>Open Dialog</nu-button>
        `;
      }
    }
    ...
    const dialog = container.shadowRoot?.querySelector('nu-dialog');

    if (!dialog) {
      throw Error('Could not find <nu-dialog> element.');
    }
    ...
    // Opens modal.
    const openModalButton = container.shadowRoot?.querySelector('nu-button');
