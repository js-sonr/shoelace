...
import type NuFormatBytes from './format-bytes.js';

describe('<nu-format-bytes>', () => {
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<NuFormatBytes>(html` <nu-format-bytes></nu-format-bytes> `);
  ...
  describe('bits', () => {
    ...
    results.forEach(expected => {
      it('bits : display formats', async () => {
        const el = await fixture<NuFormatBytes>(html` <nu-format-bytes unit="bit"></nu-format-bytes> `);
        // short
        el.value = expected.value;
        await elementUpdated(el);
        expect(el.shadowRoot?.textContent).to.equal(expected.short);
