...
const waitForScrollButtonsToBeRendered = async (tabGroup: NuTabGroup): Promise<void> => {
  await waitUntil(() => {
    const scrollButtons = tabGroup.shadowRoot?.querySelectorAll('nu-icon-button');
    return scrollButtons?.length === 2;
  });
};

const getClientRectangles = (tabGroup: NuTabGroup): ClientRectangles => {
...
const expectHeaderToBeVisible = (container: HTMLElement, dataTestId: string): void => {
  const generalHeader = queryByTestId<NuTab>(container, dataTestId);
...
const expectOnlyOneTabPanelToBeActive = async (container: HTMLElement, dataTestIdOfActiveTab: string) => {
  await waitUntil(() => {
    ...
    const activeTabPanels = tabPanels.filter((element: NuTabPanel) => element.hasAttribute('active'));
...
const expectPromiseToHaveName = async (showEventPromise: Promise<NuTabShowEvent>, expectedName: string) => {
  const showEvent = await showEventPromise;
  expect(showEvent.detail.name).to.equal(expectedName);
};

const waitForHeaderToBeActive = async (container: HTMLElement, headerTestId: string): Promise<NuTab> => {
  const generalHeader = queryByTestId<NuTab>(container, headerTestId);
  await waitUntil(() => {
...
describe('<nu-tab-group>', () => {
  ...
  it('displays all tabs', async () => {
    const tabGroup = await fixture<NuTabGroup>(html`
      <nu-tab-group>
        <nu-tab slot="nav" panel="general" data-testid="general-tab-header">General</nu-tab>
        <nu-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</nu-tab>
        <nu-tab-panel name="general">This is the general tab panel.</nu-tab-panel>
  ...
  it('shows the first tab to be active by default', async () => {
    const tabGroup = await fixture<NuTabGroup>(html`
      <nu-tab-group>
        ...
        <nu-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</nu-tab-panel>
  ...
  describe('scrolling behavior', () => {
    const generateTabs = (n: number): HTMLTemplateResult[] => {
      ...
      for (let i = 0; i < n; i++) {
        result.push(
          html`<nu-tab slot="nav" panel="tab-${i}">Tab ${i}</nu-tab>
            <nu-tab-panel name="tab-${i}">Content of tab ${i}0</nu-tab-panel> `
        );
      }
      return result;
    };
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<NuTabGroup>(html`<nu-tab-group> ${generateTabs(30)} </nu-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<NuTabGroup>(html`<nu-tab-group> ${generateTabs(2)} </nu-tab-group>`);
    ...
    it.skip('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<NuTabGroup>(html`<nu-tab-group> ${generateTabs(50)} </nu-tab-group>`);
      tabGroup.placement = 'start';
    ...
    it.skip('does scroll on scroll button click', async () => {
      ...
      const tabGroup = await fixture<NuTabGroup>(
        html`<nu-tab-group> ${generateTabs(numberOfElements)} </nu-tab-group>`
      );
      ...
      const scrollToRightButton = tabGroup.shadowRoot?.querySelector('nu-icon-button[part*="scroll-button--end"]');
  ...
  describe('tab selection', () => {
    const expectCustomTabToBeActiveAfter = async (tabGroup: NuTabGroup, action: () => Promise<void>): Promise<void> => {
      const generalHeader = await waitForHeaderToBeActive(tabGroup, 'general-header');
      generalHeader.focus();

      const customHeader = queryByTestId<NuTab>(tabGroup, 'custom-header');
      expect(customHeader).not.to.have.attribute('active');

      const showEventPromise = oneEvent(tabGroup, 'sl-tab-show') as Promise<NuTabShowEvent>;
      await action();
    ...
    it('selects a tab by clicking on it', async () => {
      const tabGroup = await fixture<NuTabGroup>(html`
        <nu-tab-group>
          <nu-tab slot="nav" panel="general" data-testid="general-header">General</nu-tab>
          <nu-tab slot="nav" panel="custom" data-testid="custom-header">Custom</nu-tab>
          <nu-tab-panel name="general">This is the general tab panel.</nu-tab-panel>
          <nu-tab-panel name="custom" data-testid="custom-tab-content">This is the custom tab panel.</nu-tab-panel>
        </nu-tab-group>
      `);
    ...
    it('does not change if a disabled tab is clicked', async () => {
      const tabGroup = await fixture<NuTabGroup>(html`
        <nu-tab-group>
          <nu-tab slot="nav" panel="general" data-testid="general-header">General</nu-tab>
          <nu-tab slot="nav" panel="disabled" data-testid="disabled-header" disabled>disabled</nu-tab>
          <nu-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</nu-tab-panel>
          <nu-tab-panel name="disabled">This is the disabled tab panel.</nu-tab-panel>
        </nu-tab-group>
      `);
    ...
    it('does not allow selection of disabled tabs with arrow keys', async () => {
      const tabGroup = await fixture<NuTabGroup>(html`
        <nu-tab-group>
          ...
          <nu-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</nu-tab-panel>
          <nu-tab-panel name="disabled">This is the custom tab panel.</nu-tab-panel>
        </nu-tab-group>
      `);
