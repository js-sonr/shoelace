...
// event requires it)
type NuEventInit<T> = T extends keyof GlobalEventHandlersEventMap
  ? GlobalEventHandlersEventMap[T] extends CustomEvent<Record<PropertyKey, unknown>>
    ? GlobalEventHandlersEventMap[T] extends CustomEvent<Record<PropertyKey, never>>
...
export default class NebulaElement extends LitElement {
  ...
  static version = __NEBULA_VERSION__;
  /* eslint-enable */

  static define(name: string, elementConstructor = this, options: ElementDefinitionOptions = {}) {
    const currentlyRegisteredConstructor = customElements.get(name) as CustomElementConstructor | typeof NebulaElement;
    ...
    if (!currentlyRegisteredConstructor) {
      ...
      // Some tools like Jest Snapshots and if you import the constructor and call `new NuButton()` they will fail with
  ...
  constructor() {
    super();
    Object.entries((this.constructor as typeof NebulaElement).dependencies).forEach(([name, component]) => {
      (this.constructor as typeof NebulaElement).define(name, component);
    });
  }
}
